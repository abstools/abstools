...
zero-argument functions of type ~String~ or ~Int~, which are exported from
~Org~, so if the code uses another module it can use ~import * from Org'~ to
access parameters given to the block.
