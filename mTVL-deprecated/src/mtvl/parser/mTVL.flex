...
AttIdentifier  = ([:lowercase:] ([:letter:] | [:digit:] | "_")*)            // frontend Identifier
//               | ([:uppercase:] ([:letter:] | [:digit:] | "_" | ".")* "."
//                  [:lowercase:] ([:letter:] | [:digit:] | "_")*)
FeatIdentifier = [:uppercase:] ([:letter:] | [:digit:] | "_")*              // frontend TypeIdentifier
...
<YYINITIAL> {
  {AttIdentifier}   { return sym(Terminals.AID); }    // now IDENTIFIER
  {FeatIdentifier}  { return sym(Terminals.FID); }    // now TYPE_IDENTIFIER
  {IntLiteral}      { return sym(Terminals.INTLIT); } // now INTLITERAL, bools return BOOLITERAL
