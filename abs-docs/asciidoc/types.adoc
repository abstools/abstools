...
Exceptions are declared with the keyword `exception`.  Exceptions are of type
`ABS.StdLib.Exception`, which is pre-defined in the standard library.
...
.Syntax
|====
| _ExceptionDecl_ ::= | `exception` _SimpleTypeIdentifier_ `;`
|====
...
NOTE: Unlike normal datatype constructors, exceptions cannot take
arguments yet.  This feature is planned for a future version of ABS.
