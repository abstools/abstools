...
def Process find_shortest_deadline(Process candidate, List<Process> l) =
  ...
  else if durationLessThan(procDeadline(head(l)), procDeadline(candidate))
...
|`procDeadline` |`Duration` | The (current) deadline value of the process (`InfDuration` if none)
|=======================
