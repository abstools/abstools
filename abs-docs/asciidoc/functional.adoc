...
.Syntax
|====
| _TypeCheckExp_ ::= | _PureExp_ `is` _TypeIdentifier_
|====
...
Variables pointing to objects are typed by interface, which means that the
concrete class of the referenced object might support more methods than can be
called through the reference.  The type-cast expression returns an
<<type-optionals,Optional>> of type `Maybe<I>` that wraps the object if it
implements the given interface `I`, or `Nothing` otherwise.
...
{
  I o = new C();
  Maybe<J> mj = o as J;  // fromJust(mj) is of type J
}
----
