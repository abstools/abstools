...
If no `DC` annotation is given for a `new` statement, the fresh cog
...
Decrease the total available amount for the given resourcetype by `amount` from
...
Increase the total available amount for the given resourcetype by `amount` from
