...
defined with pure expressions, for example the function `println`.  The
definition of such functions is done via a special function body of the form
`builtin`, with optional pure expression arguments `builtin(a, "b", 3)`.  Such
builtin functions have to be defined separately in the code generator for each
backend where the model is compiled; details of this are
implementation-specific and outside the scope of this manual.
