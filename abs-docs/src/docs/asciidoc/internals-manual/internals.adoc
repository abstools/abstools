...
`org.abs_models.backend.java.scheduling.DefaultTaskScheduler` but this is
customizable by the user when starting a model.
...
handled by a Java thread of this class.  `SchedulerThread` instances
`wait` on the enclosing DefaultTaskScheduler instance when they are
ready to run.  `SchedulerThread` instances are responsible for calling
`notifyAll` when they suspend or exit.

A `SchedulerThread` instance is created either by the method
`SchedulerThread.addTaskToScheduler` (if no task is running), or by a running
