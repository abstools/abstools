...
The `takeMaybe` function can be used to iterate through a set as follows:
...
Alternatively, `map` and other functions that operate on lists can be
used on sets by obtaining a list of the set's elements via the
`elements` function.
...
type `Pair<A, B>` to the `map` constructor function.  The keys of the
resulting map are of type `A` and values are of type `B`.  The
...
Returns the value associated with a given key `k`.  If the map does not contain an
entry with key `k`, return the default value `d`.
...
NOTE: If you need to know whether the map actually contains an entry
for key `k`, even if it maps to `d`, consider using the function
`lookup` instead.
...
Given a value `v`, if there is an entry `Pair(k,v)` in the map return
`Just(k)`.  Otherwise, return `Nothing`.
...
Given a value `v`, if there is an entry `Pair(k,v)` in the map return
`k`.  Otherwise, return the default value `d`.
...
NOTE: If you need to know whether the map actually contains an entry
with value `v`, even if its key is `d`, consider using the function
`lookupReverse` instead.
