...
.Syntax
====
_DataTypeDecl_ ::= `data` _TypeIdentifier_ [ _TypeParams_ ] [ `=` _DataConstr_ { `|` _DataConstr_ } ] `;` +
_TypeParams_ ::= `<` _TypeIdentifier_ { `,` _TypeIdentifier_ } `>` +
_DataConstr_ ::= _TypeIdentifier_ [ `(` _Type_ [ _Identifier_ ] { `,` _Type_ [ _Identifier_ ] } `)` ]
====
...
.Syntax
====
_TypeSynDecl_ ::= `type` _TypeIdentifier_ `=` _Type_ `;`
====
