...
.Syntax
====
_InterfaceDecl_ ::= `interface` _TypeIdentifier_ [ `extends` _TypeIdentifier_ { `,` _TypeIdentifier_ } ] `{` { MethSig } `}` +
...
.Syntax
====
_ClassDecl_ ::= `class` _TypeIdentifier_ [ `(` [ _Type_ _Identifier_ { `,` _Type_ _Identifier_ } ] `)` ] +
[ `implements` _TypeIdentifier_ { `,` _TypeIdentifier_ } ] +
