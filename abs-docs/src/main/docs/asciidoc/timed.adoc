...
[[fig-time-advance]]
image:ABS-time-advance.png[width=510,height=150,title="Time Advance",alt="Time Advance"]

Figure <<fig-time-advance>> shows two process, `P1` and `P2`.  They are
waiting for time to advance between `(4, 6)` and `(3, 5)` units, respectively.
Assuming that no other process is ready to run, the clock will advance the
maximum amount that still hits the earliest interval, in this case 5.  Since
the clock is now within both intervals, both processes will be unblocked and
ready to run.
