        ...
        return (new abs.backend.java.lib.expr.Case() {
            public ABS.StdLib.Set<A> of(final A e, final ABS.StdLib.Set<A> xs, final ABS.StdLib.Set<A> __ABS_value) {
                ...
                if (__ABS_binding2 != null) return new Object() {
                    public ABS.StdLib.Set<A> execute(final A x, final ABS.StdLib.Set<A> ss) { return abs.backend.java.lib.expr.BinOp.lt(e,x).toBoolean() ? new ABS.StdLib.Set_Insert(e, xs) : new ABS.StdLib.Set_Insert(x, ABS.StdLib.insertElement_f.apply(ss, e)); }
                }.execute((A) __ABS_binding2.getBinding(0),(ABS.StdLib.Set<A>) __ABS_binding2.getBinding(1));
                throw new abs.backend.java.lib.expr.UnmatchedCaseException("abslang.abs:227:2:  value " + __ABS_value + " did not match any pattern.");
            }
        }.of(e, xs, xs));
    }
}
