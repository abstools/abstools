...
public class Config {
   ...
   public static final ConfigOption LOGLEVEL_OPTION = newOption("abs.loglevel","sets a logging level",OptionType.STRING);
   public static final ConfigOption DEBUG_OPTION = newOption("abs.debug","enables debugging",OptionType.BOOLEAN);
   public static final ConfigOption SYSTEM_OBSERVER_OPTION = newOption("abs.systemobserver","sets a system observer class",OptionType.CLASS);
   public static final ConfigOption TOTAL_SCHEDULER_OPTION = newOption("abs.totalscheduler","sets a total scheduler class",OptionType.CLASS);
   public static final ConfigOption GLOBAL_SCHEDULER_OPTION = newOption("abs.globalscheduler","sets a global scheduler class",OptionType.CLASS);
   	...
   	if (Boolean.parseBoolean(System.getProperty("abs.help","false"))) {
      	...
      	System.err.println(" ================================================");
   		for (ConfigOption o : options) {
   			System.err.println(String.format("%36s\t%s","-D"+o.propertyName+"=<"+o.type.toString().toLowerCase()+">",o.description));
   		}
      	System.exit(1);
   	}
   }
   ...
   static {
       ...
       boolean recording = Boolean.parseBoolean(System.getProperty(RECORD_TASK_SCHEDULER_OPTION.propertyName,"false"));
       if (recording) {
   ...
   static {
       ...
       if (globalSchedulerStrat != null) {
           logger.config("Using global scheduling strategy defined by class "+globalSchedulerStrat.getClass().getName());
   ...
   private static TaskSchedulerFactory getSchedulerFactory() {
       ...
       if (schedulerName.equals("default"))
           ...
           return SimpleTaskScheduler.getFactory();
       logger.warning("The task scheduler "+schedulerName+" does not exist, falling back to the default task scheduler.");
       return DefaultTaskScheduler.getFactory();
   }
