...
start()->
    case init:get_plain_arguments() of
        []->
            run_mod(?ABSMAINMODULE, false, false, none, none, maps:new(), false, false);
        Args->
            start(Args)
   end.
...
parse(Args,Exec)->
    case getopt:parse_and_check(?CMDLINE_SPEC,Args) of
        {ok,{Parsed,[]}} ->
            ...
            run_mod(Module, Verbose, Debug, Port, Clocklimit, ReplayTrace, DumpTrace, ExoMode);
        _ ->
...
run_mod(Module, Verbose, Debug, Port, Clocklimit, Trace, DumpTrace, ExoMode)  ->
    case Port of
