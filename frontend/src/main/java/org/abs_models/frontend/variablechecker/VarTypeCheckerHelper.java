...
public class VarTypeCheckerHelper {
    ...
    public static void checkAssignment(SemanticConditionList l, ASTNode<?> n, Type lht, Exp rhte, ModelFamilySignature signature) {
        Type te = rhte.getType();

        if(te.isUnknownType() && rhte instanceof VarOrFieldUse){
            te = VarTypeCheckerHelper.devar(((VarOrFieldUse)rhte).getDecl(), signature);
        }
        if (!te.varIsAssignableTo(lht, signature)) {
            boolean debug = te.varIsAssignableTo(lht, signature);
    ...
    public static void typeCheckMatchingParams(SemanticConditionList l, DataConstructorExp n, DataConstructor c, ModelFamilySignature signature) {
        assert n.getDecl() == c;
        final Map<TypeParameter, Type> binding = n.getTypeParamBinding(n, c);
        typeCheckEqual(l, n, c.applyBindings(binding), signature);
    }
    ...
    public static void typeCheckMatchingParams(SemanticConditionList l, FnApp n, ParametricFunctionDecl decl, ModelFamilySignature signature) {
        Map<TypeParameter, Type> binding = n.getTypeParamBindingFromParamDecl(decl);
        java.util.List<Type> types = decl.applyBindings(binding);
        typeCheckEqual(l, n, types, signature);
    }

    public static void typeCheckEqual(SemanticConditionList l, ASTNode<?> n, java.util.List<Type> params, ModelFamilySignature signature) {
        ...
        if (params.size() != args.getNumChild()) {
            ...
            for (int i = 0; i < params.size(); i++) {
                ...
                if (nerrors == l.getErrorCount()) {
                    ...
                    if (!expType.varIsAssignableTo(argType, signature)) {
    ...
    public static void addTypeParamBinding(ASTNode<?> node, Map<TypeParameter, Type> binding, java.util.List<Type> params,
            java.util.List<Type> args, ModelFamilySignature signature) {
        ...
        for (int i = 0; i < params.size(); i++) {
            ...
            if (paramType.isTypeParameter()) {
                if (binding.containsKey(paramType)) {
                    ...
                    if (prevArgType.varIsAssignableTo(argType, signature)
                        && !argType.varIsAssignableTo(prevArgType, signature))
                    {
                ...
                if (paramdt.numTypeArgs() == argdt.numTypeArgs()) {
                    addTypeParamBinding(node, binding, paramdt.getTypeArgs(), argdt.getTypeArgs(), signature);
                }
            }
        }
    }
    ...
    public static Type devar(VarOrFieldDecl decl, ModelFamilySignature signature){

        if(decl instanceof VarDecl){
            VarDecl ddecl = (VarDecl) decl;
            Access ast = ddecl.getAccess();
            if(ast instanceof VariableInterfaceTypeUse) {
                return ((InterfaceFamilySignature) signature.resolve(((VariableInterfaceTypeUse)ast).getName())).toType();
            }
        }
        if(decl instanceof FieldDecl){
            FieldDecl ddecl = (FieldDecl) decl;
            Access ast = ddecl.getAccess();
            if(ast instanceof VariableInterfaceTypeUse) {
                return  ((InterfaceFamilySignature) signature.resolve(((VariableInterfaceTypeUse)ast).getName())).toType();
            }
    ...
    public static void checkDefBeforeUse(SemanticConditionList e, VarOrFieldUse use, ModelFamilySignature signature) {
        if (use.getType().isUnknownType()) {
            boolean fail = false;
            if(use.getDecl() instanceof VarDecl){
                VarDecl decl = (VarDecl) use.getDecl();
                Access ast = decl.getAccess();
                if(ast instanceof VariableInterfaceTypeUse) {
                    Type t = ((InterfaceFamilySignature) signature.resolve(((VariableInterfaceTypeUse)ast).getName())).toType();
                    if(t == null || t.isUnknownType()) fail = true;
                }
            }
            if(use.getDecl() instanceof FieldDecl){
                FieldDecl decl = (FieldDecl) use.getDecl();
                Access ast = decl.getAccess();
                if(ast instanceof VariableInterfaceTypeUse) {
                    Type t = ((InterfaceFamilySignature) signature.resolve(((VariableInterfaceTypeUse)ast).getName())).toType();
                    if(t == null || t.isUnknownType()) fail = true;
