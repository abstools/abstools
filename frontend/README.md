...
To build the compiler, run `make frontend` in the project root
directory, or `../gradlew assemble` in this subdirectory
(`..\gradlew.bat assemble` on windows).  A successful build generates
the file `dist/absfrontend.jar` which contains the compiler and
runtime support files.  A convenience script called `absc` that
invokes the ABS compiler can be found in `bin/bash/` (for Unix-like
systems) and `bin/win` (for Windows).  For a list of options to the
compiler, see the output of `absc -help`.
